...
const CompletionScreen: React.FC<CompletionScreenProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in zoom-in duration-300 bg-black min-h-dvh">
      <div className="w-full max-w-sm mx-auto p-6 inset-ring-1 inset-ring-blue-500/30 inset-shadow-xs inset-shadow-white/50 border border-zinc-800 rounded-2xl bg-zinc-900/40">
        <div className={`w-20 h-20 ${bgColor} mx-auto rounded-full flex items-center justify-center mb-6`}>
          <span className="text-4xl">{emoji}</span>
        </div>
        <p className={`text-zinc-400 mx-auto ${showMissed ? 'mb-4' : 'mb-8'}`}>
          {isQuizMode ? (
        ...
        {showMissed && (
          <div className="border-y bg-zinc-900/30 border-zinc-800 mb-8 ml-[-1.5rem] mr-[-1.5rem] pl-6">
            <div className="scrollbar-thumb-indigo-700 py-4 scrollbar-track-gray-100/10 text-left max-h-48 overflow-y-auto">
